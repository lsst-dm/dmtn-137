...
Recently, the middleware layer is in the course of a major redesign and refactoring, with the new middleware known as the Generation 3 Middleware ("Gen3" Middleware; \citeds{DMTN-056}).
...
Also, the current implementations of the Butler registry depend on conflict resolution syntax to resolve insertions of complex datasets, that may not work in this scenario.
...
In the first phase Amazon EFS is used as this filesystem.
The data repository, including a POSIX datastore and a SQLite registry, was entirely located on EFS, which was mouted to each of the EC2 compute instances.
...
Keeping this in mind, we decided not to always follow the bleeding edge version and updated only when unnecessary, such as for important bug fixes.
