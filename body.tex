...
Amazon Machine Image (AMI) bundle the Operating System (OS) with required data and configurations into a convenient image that can be launched on any EC2 instance type.
...
Pegasus workflows are expressed as Directed Acyclic Graphs (DAGs) where jobs are the nodes and job dependencies are represented the edges.
...
\subsubsection{LSST Generation 3 Middleware / Data Butler?}
...
AMIs containing LSST Stack, Pegasus, configured HTCondor are pre-built in two flavors: master and worker. These contain the complete environment required to run DRP workflows and scale them out with HTCondor. The user launches an on-demand EC2 instance using the master AMI to be the HTCondor submit host and the central manager of HTCondor pool.
...
The data repository, including a POSIX datastore and a SQLite registry, was located on entirely EFS that was mouted to each of the EC2 compute instances.

Later, the S3 datastore replaced the need of EFS, and the PostgreSQL RDS replaced the SQLite registry.
Also, a Personal Condor Pool on an single EC2 instance was used before we tested the capability to launch new EC2 instances as the Condor workers and execute jobs.
...
({\color{red} not sure I understand this?})
...
Lack of a good integration test is also a concern going forward.
